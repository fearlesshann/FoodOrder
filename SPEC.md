...
- V22 后端镜像依赖安装层不得因 `app/` 源码变化失效；远程部署命令超时不少于 30 分钟。
- V23 后端镜像安装 Python 依赖时使用持久 pip 缓存，网络读取超时不少于 120 秒且至少重试 10 次。
- V24 `/uploads/` 必须优先代理到 API，不得被前端图片静态资源正则拦截。
- V25 生产站点必须声明可用 favicon；传统 `/favicon.ico` 请求不得返回 404。
- V26 手机端展开备注后输入框立即获得焦点；键盘引起的程序性滚动不得收起，真实触摸滑动仍须保存并收起。
- V27 手机软键盘改变可视视口高度时，当前备注输入框必须自动保持在可视区域内。
- V28 菜品图片支持 JPEG、PNG、WebP、HEIC、HEIF，单文件不超过 20MB；手机照片按 EXIF 方向校正后转为 WebP。
- V29 分类筛选的菜品离场动画必须脱离文档流；筛选期间分类工具栏与列表起始位置不得上下跳动。
- V30 首页与菜单页切换时，新页面必须立即挂载；固定底部主按钮随页面同步出现，不得叠加独立入场延迟。
- V31 页面切换动效仅由旧页面退场承担；新页面根节点不得使用会影响固定底部主按钮的入场 transform、filter 或 opacity。
- V32 首页与菜单页的固定底部主按钮必须渲染在页面过渡根节点之外；页面内容保留双向进出场动效，按钮不继承该动效。
...
B5|2026-07-25|后端 Dockerfile 在安装依赖前复制业务源码，重建时重复下载依赖并超过 SSH Action 10 分钟限制|V22
B6|2026-07-25|服务器访问 PyPI 索引超过 pip 默认 15 秒读取超时，依赖解析误报无可用版本且失败下载无法复用|V23
B7|2026-07-25|Nginx 图片扩展名正则优先于普通 `/uploads/` 前缀，上传图片被错误地从前端静态目录读取并返回 404|V24
B8|2026-07-25|前端未提供或声明 favicon，浏览器默认请求 `/favicon.ico` 返回 404|V25
B9|2026-07-25|动态 textarea 仅依赖 `autofocus` 且所有 scroll 事件都触发收起，手机键盘无法稳定弹出并会因焦点滚动立即关闭|V26
B10|2026-07-25|备注聚焦时阻止页面滚动但未监听软键盘造成的可视视口缩小，输入框初次被键盘覆盖|V27
B11|2026-07-25|上传仅支持 8MB 内的 JPEG/PNG/WebP，手机原片常见的 HEIC/HEIF 或较大 JPEG 被拒绝，而截图可正常上传|V28
B12|2026-07-26|分类筛选时离场菜品仍占据文档流，与入场菜品同时短暂撑高列表并造成上下跳动|V29
B13|2026-07-26|页面使用串行 `out-in` 过渡，首页底部按钮又叠加独立 dock 入场，导致两页主按钮延迟且时序不一致|V30
B14|2026-07-26|新页面根节点仍执行 transform/filter 入场，菜单页固定底部按钮继承该效果并继续表现为延迟显示|V31
B15|2026-07-26|为消除固定按钮延迟而移除了新页面全部入场样式，导致页面切换失去可见动效|V32
